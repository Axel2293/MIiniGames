...
// GENRAL struct for the leaderboard
...
int search_user(table_lb game_lb, char search[])
{
  for(int i=0; i<game_lb.free_indx; i++)
  {
    if (strcmp(game_lb.player[i].name,search) == 0)
    {
      return i;
    }
  }
...
{    
  ...
  switch(res){
    ...
    case 2:
      ...
      scanf("%s", search);
      getchar();
      int indx=search_user(game_lb,search);
      if(indx!=-1){
...
table_lb Ahorcado(table_lb ahorcado_lb)
{
  ...
  switch(res){
    ...
    case 2:
      ...
      scanf("%s", search);
      getchar();
      int indx=search_user(ahorcado_lb,search);
      if(indx!=-1){
...
table_lb blackjack(table_lb bj_lb)
{
  ...
  switch(res){
    ...
    case 2:
      ...
      scanf("%s", search);
      getchar();
      int indx=search_user(bj_lb,search);
      if(indx!=-1){
  ...
  while(1)
  {
    ...
    // CHECK IF PLAYER LOST OR WON WITH A BLACKJACK THIS PART JUST BREAK THE MAINLOOP
    if(chek_21(player_deck)>21 )
    {
      break;
    }
